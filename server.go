...
func handleConn(p1 net.Conn) {
	...
	for {
		...
		case cmdLOGIN: // login ID+pass
			// ID = '11111111111' password = '123'
			//[server 4003] 6500 f003 2b23 00000000313131313131313131313100003461636331333165373232313432383265663136336633643266613835636363003132322e39362e34322e36330000000000aef53323000000000000044a61636b00000000000000000000000000040d00000600f003419c85350000
...
func handleUser(p1 *Client, buffer []byte) {
	...
	for {
		...
		/*
		   unhandled situition:
		   enters senior channel 1
		   2019/10/30 23:39:16 [????] [0A87][0006][03F0]06 00 F0 03 87 0A 00 00 00 00
		   2019/10/30 23:39:16 [????] [0679][0022][03F0]16 00 F0 03 79 06 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 01 00 00 00
		*/
		case cmdHERT:
			// [6F][0002][03F0]02 00 F0 03 6F 1F
			Respond(p1, "[heartReply]", heartReply)
		...
		case 0x0621:
			...
			//f.data is from 5th byte to the end, so 211c message will match this 0x21
			//payload -> 00000000211c0000
		...
		case 0x0847:
			//if bytes.Compare(f.payload[8:10], []byte{byte(0x5f), byte(0x24)}) == 0 {
			//	Vln(4, "client requested pvp channel 5f24")
			//} else if bytes.Compare(f.payload[8:10], []byte{byte(0x5b), byte(0x24)}) == 0 {
			//	Vln(4, "client requested pvp channel 5b24")
			//}
		...
		case 0x080E:
			// server response of server room names

			//p1.WriteRawFrame("08 00 F0 03 C8 0A 85 35 00 00 02 00")

			//this response shows empty room
			//p1.RespondRawFrame("[0x080E]", "04 00 F0 03 DA 06 00 00")

			p1.RespondRawFrame("[0x080E]", channelRoomResponse1)
			p1.RespondRawFrame("[0x080E]", channelRoomResponse2)
			...
			/*
					case ??: login pvp channel 1
				   <- [14][f003][timestamp_or_udp_ip_port?:4708] 00000000000000005b240000
				  server|0e00f003ad07000000008e23682423000000
				   -> [14][f003][????:ad07----------------] 000000008e23682423000000
				  client|0a00f0036b070000000003000000
				   <- [10][f003][login_page_ok:6b07-------] 0000000003000000
				  server|0a00f00337068535000003000000
				   -> [10][f003][server_login_page_ok:3706] 8535000003000000
				  client|0600f003600800000000
				   <- [6][f003][????:6008----------------] 00000000
				  server|1600f0032c0785350000e30701000d00140005001e000b010000
				   -> [22][f003][????:2c07----------------] 85350000e30701000d00140005001e000b010000
				  client|0600f003870a00000000
				   <- [6][f003][????:870a----------------] 00000000
				  server|1600f0032c07986d0000e2070c0016000f002a002200ef010000
				   -> [22][f003][????:2c07----------------] 986d0000e2070c0016000f002a002200ef010000
				  client|0600f0036908000000000600f003250600000000
				   <- [6][f003][????:6908----------------] 000000000600f003250600000000
				  server|0e00f0033507853500000000000000000000
				   -> [14][f003][????:3507----------------] 853500000000000000000000
				  server|0800f003c80a853500000200
				   -> [8][f003][????:c80a----------------] 853500000200
				  client|1600f00379060000000001000000000000000000000001000000
				   <- [22][f003][????:7906----------------] 0000000001000000000000000000000001000000
				  server|1600f0032c07986d0000e2070c0016000f002a002200ef010000
				   -> [22][f003][????:2c07----------------] 986d0000e2070c0016000f002a002200ef010000
				  client|0200f0030e08
				   <- [2][f003][????:0e08----------------]
				  server|0400f003da060000
				   -> [4][f003][????:da06----------------] 0000
